...
export default function Form() {
    ...
    const attemptSubmit = async (submitEvent: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const addMemberResponse = await (await fetch('/api/mailchimp', {
                method: 'POST',
    ...
    return (
        isLoading ?
            <div className='flex justify-center items-center py-8'>
                <ClipLoader color='brand_black' speedMultiplier={1} size={80} className='mx-auto' />
            </div>
            :
        isCompleted && subscriptionResponse ?
            <div className='flex flex-col justify-start items-center h-32 mx-auto'>
                {subscriptionResponse.success ? 
                    ...
                    <>
                        ...
                        <p dangerouslySetInnerHTML={{__html:subscriptionResponse.message}}></p>
                    </>
                } 
            </div>
            :
                ...
                <fieldset className='flex flex-col gap-2'>
                    ...
                    <label className='flex flex-col w-[90%]'>
                        <input 
                            ...
                            className={`border-2 border-solid rounded-md border-brand_grey focus:border-brand_gold p-1 text-sm ${errors.firstName ? "border-red-500" : ""}`}
                        />
                    ...
                    <label className='flex flex-col w-[90%]'>
                        <input
                            ...
                            onChange={handleChange}
                            className={`border-2 border-solid rounded-md border-brand_grey focus:border-brand_gold p-1 text-sm ${errors.lastName ? "border-red-500" : ""}`}
                        />
                ...
                <fieldset className='flex flex-col gap-2'>
                    ...
                    <label className='row-start-2 col-span-1 flex flex-col w-[90%]'>
                        <input 
                            ...
                            onChange={handleChange}
                            className={`border-2 border-solid rounded-md border-brand_grey focus:border-brand_gold p-1 text-sm ${errors.email ? "border-red-500" : ""}`}
                        />
                            ...
                            {["1L", "2L", "3L"].map((year) => (
                                <span key={year} className='text-sm flex flex-col'>
                                    ...
                                    <input
                                        ...
                                        id={year}
                                        onClick={e => {
                                            const selectedYear = (e.target as HTMLInputElement).value as "1L" | "2L" | "3L";
...
const formSchema = z.object({
    ...
    email: z.string().min(1, { message: "Email is required"}).email({ message: "Invalid email format"}).endsWith('@fordham.edu', { message: "Use your '@fordham.edu' email"}),
    year: z.enum(["1L", "2L", "3L"], { required_error: "You must select a year"})
});
